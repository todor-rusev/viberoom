...
(function () {
  ...
  async function open(initialRoom, helpers) {
    ...
    let inspected, selected = [], dependencyChoices = {}, preview, fileName = "", exportSelection, exportParts = { conversation: true, settings: true, resources: true }, newLabel;
    ...
    function updateSizes() {
      for (const key of Object.keys(exportParts)) {
        const total = estimates.filter(r => exportSelection.has(r.id)).reduce((sum, r) => sum + (key === "conversation" ? r.conversationBytes : key === "settings" ? r.settingsBytes : r.resourceBytes), 0);
        const el = $(`[data-part-size="${key}"]`); if (el) el.textContent = estimates.length ? bytes(total) : "measuring…";
      }
    }
    function renderExport() {
      ...
      content.innerHTML = `<form class="carry-export-form">
        ...
        <fieldset class="carry-part-list"><legend>What to carry</legend>${[["conversation", "Conversation and removed versions"], ["settings", "Room rules, vibemates and their skills"], ["resources", "Attached pictures and documents"]].map(([key, label]) => `<label><input type="checkbox" data-export-part="${key}"${exportParts[key] ? " checked" : ""}><span>${label}</span><small data-part-size="${key}"></small></label>`).join("")}</fieldset>
    ...
    function renderSelection() {
      ...
      content.innerHTML = `<p>From <strong>${esc(inspected.source.label || "an unnamed copy")}</strong>. Choose the destination and the parts for each room.</p>${!catalog.source.label ? `<label class="carry-field">Name this copy of viberoom<input data-local-copy-name maxlength="100" value="${esc(catalog.source.suggested)}"><span class="hint">Kept for later transfers, so you can recognise this branch.</span></label>` : ""}${inspected.rooms.map((r, i) => {
        ...
        return `<fieldset class="carry-import-room" data-import-room="${i}"><legend><label><input type="checkbox" data-selected${choice.selected ? " checked" : ""}>${esc(r.name)}</label></legend>
          ...
          <div class="carry-inline-parts">${["conversation", "settings", "resources"].map(part => `<label><input type="checkbox" data-import-part="${part}"${choice[part] ? " checked" : ""}${r.parts.includes(part) ? "" : " disabled"}>${part === "settings" ? "Room setup" : part === "resources" ? "Resources" : "Conversation"}</label>`).join("")}</div>
    ...
    function takeSelection() {
      for (const field of dialog.querySelectorAll("[data-import-room]")) {
        ...
        for (const part of ["conversation", "settings", "resources"]) choice[part] = field.querySelector(`[data-import-part="${part}"]`).checked;
        delete choice.branch; delete choice.setup; delete choice.resourcesChoice;
      }
    }
    function renderPreview() {
      phase = "preview"; clearError();
      const unresolved = !preview.ready;
      content.innerHTML = `${preview.rooms.map(r => {
        ...
        <div class="actions">${button("back-selection", "Change rooms or parts")}${button(unresolved ? "resolve" : "apply", unresolved ? "Check these choices" : "Bring in these rooms", true)}</div>`;
      ...
      for (const select of dialog.querySelectorAll("[data-branch], [data-setup], [data-resource-choice], [data-dependency]")) select.addEventListener("change", () => {
    ...
    async function checkPlan() {
      ...
      if (!choices.length) throw new Error("Choose at least one room.");
      if (choices.some(c => !c.conversation && !c.settings && !c.resources)) throw new Error("Choose at least one part for each selected room.");
      ...
      await request(`/api/carry/${job}/plan`, { rooms: choices, dependencies: dependencyChoices });
    ...
    async function inspectedState(state) {
      ...
      selected = inspected.rooms.map(r => {
        ...
        return { uuid: r.uuid, selected: true, target: match?.id || null, name: r.name, conversation: r.parts.includes("conversation"), settings: r.parts.includes("settings"), resources: r.parts.includes("resources") };
      });
      renderSelection();
    }
    ...
    async function act(action, target) {
      ...
      try {
        ...
        if (action === "export") { keepExportInputs(); renderExport(); }
          ...
          keepExportInputs();
          if (!exportSelection.size || !Object.values(exportParts).some(Boolean)) throw new Error("Choose rooms and at least one part to carry.");
          ...
          phase = "applying"; content.innerHTML = '<p role="status">Bringing in the reviewed rooms…</p><p class="hint">The import is now being applied. Wait for the result.</p>';
          try {
            const result = await request(`/api/carry/${job}/apply`, { previewToken: preview.previewToken });
            phase = "applied"; content.innerHTML = `<p>Imported ${result.rooms.map(r => esc(r.name)).join(", ")}.</p><p class="hint">Removed and replaced variants are available under “Removed versions”.</p>`;
