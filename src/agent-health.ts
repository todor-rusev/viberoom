...
export type LoginState = "ok" | "configured" | "missing" | "unknown";
...
export function loginState(probe: LoginProbe | undefined, evidence: LoginEvidence): LoginState {
  ...
  if (probe.env.some((name) => (evidence.env[name] ?? "").trim().length > 0)) return "configured";
  if (probe.files.some((file) => evidence.exists(file))) return "configured";
